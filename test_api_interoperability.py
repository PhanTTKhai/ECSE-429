...
class TestAPIInteroperability(unittest.TestCase):
    ...
    def test_post_todo_category_succeeds_with_title(self):
        ...
        self.create_category({"title": category_title})

        # create relationship using 'title'
        link_response = requests.post(f"{self.BASE_URL}/todos/{todo_id}/categories", json={"title": category_title},
                                      headers=self.json_headers)
        self.created_category_ids.append(link_response.json()['id'])
        self.assertEqual(link_response.status_code, 201,
                         f"Expected success when linking with title, but got: {link_response.status_code}")
    ...
    def test_post_todo_category_no_todo(self):
        ...
        link_invalid_todo_response = requests.post(f"{self.BASE_URL}/todos/{non_existent_todo_id}/categories",
                                                   json={"title": category_title}, headers=self.json_headers)
    ...
    def test_post_todo_category_no_category(self):
        ...
        link_invalid_category_response = requests.post(f"{self.BASE_URL}/todos/{todo_id}/categories",
                                                       json={"title": non_existent_category_title},
                                                       headers=self.json_headers)
        self.created_category_ids.append(link_invalid_category_response.json()['id'])
        self.assertEqual(link_invalid_category_response.status_code, 404,
                         "Expected 404 when linking non-existent category to a valid todo")

    def test_post_todo_category_no_todo_no_category(self):
        non_existent_todo_id = 999999  # assuming this ID does not exist
        non_existent_category_title = "Non-existent Category"  # this category has not been created
        # expected failure: both todo and category don't exist
        link_invalid_both_response = requests.post(f"{self.BASE_URL}/todos/{non_existent_todo_id}/categories",
                                                   json={"title": non_existent_category_title},
                                                   headers=self.json_headers)
        self.assertEqual(link_invalid_both_response.status_code, 404,
                         "Expected 404 when both todo and category are non-existent")
    ...
    def test_post_todo_category_by_id(self):
        ...
        link_response = requests.post(f"{self.BASE_URL}/todos/{todo_id}/categories/{category_id}",
                                      headers=self.json_headers)
    ...
    def test_get_todo_category_by_id(self):
        ...
        get_response = requests.get(f"{self.BASE_URL}/todos/{todo_id}/categories/{category_id}",
                                    headers=self.json_headers)
    ...
    def test_post_todo_list_task_of(self):
        ...
        link_response = requests.post(f"{self.BASE_URL}/todos/{todo_id}/task-of", json={"id": project_id},
                                      headers=self.json_headers)
    ...
    def test_delete_todo_task_of(self):
        ...
        link_response = requests.post(f"{self.BASE_URL}/todos/{todo_id}/task-of", json={"id": project_id},
                                      headers=self.json_headers)
        self.assertEqual(link_response.status_code, 201,
                         f"Failed to link todo to project: {link_response.status_code} - {link_response.text}")

        delete_response = requests.delete(f"{self.BASE_URL}/todos/{todo_id}/task-of/{project_id}",
                                          headers=self.json_headers)
        self.assertEqual(delete_response.status_code, 200,
                         f"Failed to delete task-of link: {delete_response.status_code} - {delete_response.text}")
    ...
    def test_get_category_projects(self):
        ...
        link_response = requests.post(f"{self.BASE_URL}/categories/{category_id}/projects", json={"id": project_id},
                                      headers=self.json_headers)
        self.assertEqual(link_response.status_code, 201,
                         f"Failed to link category to project: {link_response.status_code} - {link_response.text}")

        get_response = requests.get(f"{self.BASE_URL}/categories/{category_id}/projects", headers=self.json_headers)
        self.assertEqual(get_response.status_code, 200,
                         f"Failed to get projects for category: {get_response.status_code} - {get_response.text}")
    ...
    def test_post_category_todo_using_title(self):  # using title
        ...
        self.create_todo({"title": "Test Todo for Category"})

        link_response = requests.post(f"{self.BASE_URL}/categories/{category_id}/todos",
                                      json={"title": "Test Todo for Category"}, headers=self.json_headers)
        self.created_todo_ids.append(link_response.json()['id'])
        self.assertEqual(link_response.status_code, 201,
                         f"Failed to link category to todo using title: {link_response.status_code} - {link_response.text}")
    ...
    def test_delete_category_todo(self):
        ...
        link_response = requests.post(f"{self.BASE_URL}/categories/{category_id}/todos", json={"id": todo_id},
                                      headers=self.json_headers)
        self.assertEqual(link_response.status_code, 201,
                         f"Failed to link category to todo: {link_response.status_code} - {link_response.text}")

        delete_response = requests.delete(f"{self.BASE_URL}/categories/{category_id}/todos/{todo_id}",
                                          headers=self.json_headers)
        self.assertEqual(delete_response.status_code, 200,
                         f"Failed to delete todo from category: {delete_response.status_code} - {delete_response.text}")
    ...
    def test_delete_non_existing_category_todo(self):
        ...
        delete_response = requests.delete(f"{self.BASE_URL}/categories/{category_id}/todos/{non_existent_todo_id}",
                                          headers=self.json_headers)
        self.assertEqual(delete_response.status_code, 404,
                         f"Expected 404 Not Found when trying to delete a non-existing relationship, but got: {delete_response.status_code} - {delete_response.text}")
    ...
    def test_post_category_id_todo_id(self):
        ...
        link_response = requests.post(f"{self.BASE_URL}/categories/{category_id}/todos/{todo_id}",
                                      headers=self.json_headers)
        self.assertEqual(link_response.status_code, 404,
                         f"Expected 404 Not Found for undocumented POST, but got: {link_response.status_code} - {link_response.text}")
    ...
    def test_get_category_id_todo_id(self):
        ...
        get_response = requests.get(f"{self.BASE_URL}/categories/{category_id}/todos/{todo_id}",
                                    headers=self.json_headers)
        self.assertEqual(get_response.status_code, 404,
                         f"Expected 404 Not Found for undocumented GET, but got: {get_response.status_code} - {get_response.text}")
